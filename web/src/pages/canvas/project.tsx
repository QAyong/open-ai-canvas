...
function InfiniteCanvasPage() {
    ...
    const handleSelectedNodeClick = useCallback((node: CanvasNodeData) => {
        ...
        } else if (node.type === CanvasNodeType.PortraitClearance) {
            setDialogNodeId(null);
    ...
    useEffect(() => {
        setNodes((current) => {
            ...
            const next = current.map((node) => {
                if (node.type !== CanvasNodeType.PortraitClearance) return node;
    ...
    const openPortraitClearance = useCallback((node: CanvasNodeData) => {
        if (node.type !== CanvasNodeType.PortraitClearance) return;
        setSelectedNodeIds(new Set([node.id]));
        setSelectedConnectionId(null);
        setContextMenu(null);
        setDialogNodeId(null);
